...
<head>
    ...
    <style>
        ...
        .card { background: #fff; width: 100%; max-width: 480px; padding: 24px; border-radius: 10px; box-shadow: 0 8px 30px rgba(0,0,0,0.08); }
        ...
        .btn { width: 100%; margin-top: 8px; padding: 10px 12px; border: none; border-radius: 6px; background: #10b981; color: #fff; cursor: pointer; font-weight: 600; }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', async function(e){
            ...
            if (!payload.name || !payload.email || !payload.password) {
                err.textContent = 'All fields are required'; err.style.display = 'block'; btn.disabled = false; btn.textContent = 'Register'; return;
            }
            try {
                const res = await fetch('auth_api.php?action=register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
<body>
    <div class="card">
        ...
        <p>All new accounts are regular users (view/search/filter only).</p>
